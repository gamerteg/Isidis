...
export function MessagesClient({ initialConversations, currentUserId, initialOrderId, targetClient }: MessagesClientProps) {
    ...
    useEffect(() => {
        if (initialOrderId && !initializedRef.current) {
            console.log('Trying to init chat with orderId:', initialOrderId)
            console.log('Available conversations:', conversations?.length)

            if (conversations) {
                const found = conversations.find(c => c.orderId === initialOrderId)
                ...
                if (found) {
                    ...
                    console.log('Creating temp conversation with client:', targetClient)
                    // Create temporary conversation if not found
                    setActiveConv({
                        otherUser: targetClient,
    ...
    }, [initialOrderId, conversations, targetClient, currentUserId])
    ...
    return (
        <div className="flex h-full gap-6">
            {/* Conversation List */}
            <div className="w-full md:w-1/3 lg:w-1/4 space-y-4 overflow-y-auto pr-2">
            ...
            {/* Chat Area */}
            <div className="hidden md:flex flex-1 bg-card/30 rounded-2xl border border-border/40 overflow-hidden relative">
                {activeConv ? (
